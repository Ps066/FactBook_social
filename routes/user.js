...
router.put('/:id', async (req,res)=>{
    ...
    if(req.body.userId === id){
        ...
        if(req.body.password){    // its important to hash newly updated pass
            try {
...
router.put('/:id/follow', async(req,res)=>{
    ...
    if(curId !== id){
        try {
            ...
            if(!user.followers.includes(curId)){  // this translates as if user dont have usrId in followers
            ...
            }else{   // condition when they are already been follwoed
                res.status(403).json("You are already following this account");
            }
        } catch (error) {
            res.status(500).json(error);
        }
    }else{   // case when we are attempting the follow ourself
...
router.put('/:id/unfollow', async(req,res)=>{
    ...
    if( curId !== id){
        ...
        try {
            ...
            if(user.followers.includes(curId)){ // condition of already follwoing
